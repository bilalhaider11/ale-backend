...
physician_api = Namespace("physician", description="Physician operations")

@physician_api.route('/upload')
class PhysicianListUpload(Resource):

    @login_required()
    @organization_required(with_roles=[PersonOrganizationRoleEnum.ADMIN])
    def post(self, person, organization):
        """
        Upload a CSV or XLSX file with physician data.
        The file will be saved to S3 with current datetime.
        """
