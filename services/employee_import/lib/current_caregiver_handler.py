...
class CurrentCaregiverHandler:
    ...
    def process_caregiver_list(self, key):
        """
        Process an caregiver CSV or XLSX file from S3
        ...
        try:
            ...
            logger.info(f"Found {len(rows)} caregiver records in file")
            
            # Delete existing records
            self.caregiver_service.delete_all_caregivers()
            
            # Import new data
            self.caregiver_service.bulk_import_caregivers(rows)
