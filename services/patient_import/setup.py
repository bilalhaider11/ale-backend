...
def setup_patient_import_queue():
    """
    Set up the SQS queue and S3 notification for patient imports
    ...
    queue_name = config.PREFIXED_PATIENT_IMPORT_PROCESSOR_QUEUE_NAME
    bucket_name = config.AWS_S3_BUCKET_NAME
    patient_prefix = f"{config.AWS_S3_KEY_PREFIX}patients-list/"
    ...
    # Step 2: Set up S3 notifications for patient prefix
    patient_setup_result = setup_s3_to_sqs_notification(
        bucket_name=bucket_name,
        queue_name=queue_name,
        s3_prefix_filter=patient_prefix
    )
    
    logger.info(f"Patient import queue setup complete")
    
    return {
        'QueueUrl': queue_url,
        'QueueArn': queue_arn,
        'QueueName': queue_name,
        'PatientS3Setup': patient_setup_result
    }
...
if __name__ == "__main__":
    ...
    setup_result = setup_patient_import_queue()
