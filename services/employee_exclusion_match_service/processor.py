...
class EmployeeExclusionMatchProcessor(BaseServiceProcessor):
    """
    Service processor that handles employee exclusion match messages
    """
    ...
    def process(self, message):
        self.logger.info("Received message: %s to the employee exclusion match service!", message)
        # Do something with the message
        # For example, you can call a method from the service's lib directory
        try:
            message_handler(message)
        except Exception as e:
            self.logger.error(f"Error running employee exclusion match service: {str(e)}")
